...
from .dagster_commands import app as dagster_app
...
# Initialize logging (already done by import)
setup_logging()

# Add sub-applications
app.add_typer(telegram_app, name="bot")
...
def evaluate(
    ...
    run_retrieval: bool = typer.Option(
        True, "--retrieval", help="Run retrieval evaluation"
    ),
    ...
    try:
        ...
        if run_retrieval:
            ...
            retrieval_evaluator = RetrievalEvaluator(
                pipeline.vector_store, pipeline.encoder
            )
...
def ingest_job(
    urls: list[str] = typer.Argument(
        ..., help="YouTube URLs to add to ingestion queue"
    ),
):
...
def run_ingestion(
    job_id: Optional[str] = typer.Option(
        None, "--job-id", help="Specific job ID to run"
    ),
...
def ingestion_status(
    ...
    try:
        ...
        if jobs:
            jobs_table = Table(
                title=f"Jobs {f'({status_filter})' if status_filter else ''}"
            )
            ...
            for job in jobs[-10:]:  # Show last 10 jobs
                ...
                jobs_table.add_row(
                    job["id"], job["status"], str(len(job["urls"])), created
                )
...
def dashboard(
    output_dir: Optional[Path] = typer.Option(
        None, "--output", help="Output directory for dashboard"
    ),
