...
try:
    ...
    print("Please install python-telegram-bot: uv pip install python-telegram-bot")
    sys.exit(1)
...
class TelegramBot:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        log_user_action(user_id, "bot_start", {"username": username})
...
• Active Tasks: {sum(1 for s in self.user_sessions.values() if s.get("current_task"))}
    ...
    async def cancel_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id in self.user_sessions and self.user_sessions[user_id].get(
            ...
            await update.message.reply_text(
                "i️ No active operation to cancel.", parse_mode=ParseMode.MARKDOWN
            )
    ...
    async def process_youtube_url(
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "✅ Yes, cancel current",
                        callback_data=f"cancel_and_start:{url}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        "❌ No, keep current", callback_data="keep_current"
                    )
                ],
            ]
        ...
        log_user_action(
            user_id, "youtube_url_submitted", {"url": url, "verbose": verbose}
        )
        ...
        try:
            ...
            log_error_context(
                e, {"user_id": user_id, "url": url, "action": "start_processing"}
            )
    ...
    async def _process_url_background(
        self, user_id: int, url: str, progress_message, verbose: bool
    ):
        """Process URL in background with progress updates."""
        try:
            # Download step
            await self._update_progress(
                progress_message, "🔽 Downloading audio...", 10, verbose
            )
            ...
            # Transcribe step
            await self._update_progress(
                progress_message, "🎤 Transcribing audio...", 40, verbose
            )
            ...
            # Process text step
            await self._update_progress(
                progress_message, "📝 Processing text...", 70, verbose
            )
            ...
            await self._update_progress(
                progress_message, "🧮 Creating embeddings...", 90, verbose
            )
            ...
            # Complete
            await self._update_progress(
                progress_message, "✅ Processing complete!", 100, verbose
            )
...
Go ahead, ask away! 🚀
            ...
            await progress_message.edit_text(
                success_text, parse_mode=ParseMode.MARKDOWN
            )
    ...
    async def _update_progress(
        self, message, status: str, percentage: int, verbose: bool
    ):
...
🔄 {status}
    ...
    async def handle_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if (
            ...
            and self.user_sessions[user_id].get("current_task") == "processing_url"
        ):
            await update.message.reply_text(
        ...
        try:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "👍", callback_data=f"feedback:good:{question[:50]}"
                    ),
                    InlineKeyboardButton(
                        "👎", callback_data=f"feedback:bad:{question[:50]}"
                    ),
                ],
                [
                    InlineKeyboardButton(
                        "🔍 More details", callback_data=f"details:{question[:50]}"
                    )
                ],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
