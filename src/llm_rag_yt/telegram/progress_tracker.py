...
class ProgressTracker:
    ...
    def start_task(self, user_id: int, task_type: str, metadata: Optional[Dict[str, Any]] = None) -> str:
    ...
    def update_progress(self, user_id: int, step: str, completed_steps: int = None, 
                       total_steps: int = None, percentage: float = None) -> bool:
        ...
        logger.debug(f"Updated progress for user {user_id}: {task.percentage:.1f}% - {step}")
        return True
    
    def complete_task(self, user_id: int, success: bool = True, error_message: Optional[str] = None):
        ...
        logger.info(f"Task {task.task_id} {'completed' if success else 'failed'} for user {user_id}")
