...
class RAGQueryEngine:
    ...
    def __init__(
        ...
        self.hybrid_search = HybridSearchEngine(vector_store, encoder) if enable_hybrid_search else None
        self.query_rewriter = QueryRewriter(model_name="gpt-4o-mini") if enable_query_rewriting else None
        self.enable_reranking = enable_reranking

        logger.info(f"Initialized RAG engine with {model_name}")
        logger.info(f"Advanced features - Hybrid: {enable_hybrid_search}, Rewriting: {enable_query_rewriting}, Reranking: {enable_reranking}")
    ...
    def query(
        ...
        try:
            ...
            logger.info(f"Generated answer for question: {question[:50]}... (took {response_time:.2f}s)")
