...
class QueryRewriter:
    ...
    def rewrite_query(
        ...
        llm_variants = self._generate_llm_variants(
            original_query, context_domain, num_variants
        )
    ...
    def _add_domain_expansions(self, query: str) -> list[str]:
        ...
        if not any(
            term in query.lower()
            for term in ["видео", "видеозапись", "video", "recording"]
        ):
        ...
        if not any(
            term in query.lower()
            for term in ["говорят", "обсуждают", "mention", "discuss"]
        ):
    ...
    def _reciprocal_rank_fusion(
        ...
        for _query_name, results in results_dict.items():
