...
class FeedbackCollector:
    ...
    def collect_feedback(
        ...
        Returns:
            Feedback ID
        """
        feedback_id = f"fb_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{abs(hash(query))}"
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            feedback.id, feedback.query, feedback.answer, feedback.rating,
            feedback.feedback_text, feedback.session_id, feedback.timestamp,
            feedback.response_time, feedback.sources_count
        ))
    ...
    def get_feedback_stats(self) -> Dict[str, Any]:
        ...
        # Basic stats
        cursor.execute("SELECT COUNT(*), AVG(rating), MIN(rating), MAX(rating) FROM feedback")
        ...
        cursor.execute("SELECT AVG(response_time) FROM feedback WHERE response_time IS NOT NULL")
