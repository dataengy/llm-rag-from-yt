...
class RetrievalEvaluator:
    ...
    def _evaluate_semantic_retrieval(
        ...
        for k in k_values:
            ...
            results["k_results"][str(k)] = {
                ...
                "success_rate": len([r for r in k_results if "error" not in r]) / len(k_results)
            }
        ...
        for k in k_values:
            ...
            for query in queries:
                try:
                    ...
                    query_embedding = self.encoder.embed_query(query)
                    semantic_docs = self.vector_store.query_similar(query_embedding, k * 2)
                    ...
                    for doc in filtered_docs:
                        original_score = 1 - doc.get("distance", 1)  # Convert distance to similarity
                    ...
                    avg_relevance = np.mean([doc["hybrid_score"] for doc in final_docs]) if final_docs else 0
                ...
                for k in k_values:
                    ...
                    for query in queries:
                        try:
                            ...
                            logger.error(f"Error with model {model_name} for '{query}': {e}")
                            k_results.append({
                                "query": query,
                                "num_results": 0,
                                "avg_relevance": 0,
        ...
        for approach_name, approach_data in approaches.items():
            if "k_results" in approach_data:
                ...
                for k, k_data in approach_data["k_results"].items():
                    score = k_data.get("avg_relevance", 0) * k_data.get("success_rate", 0)
    ...
    def run_evaluation_suite(self, output_dir: Path) -> Dict[str, Any]:
        ...
        # Save results
        output_path = output_dir / f"retrieval_evaluation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        logger.info(f"Best retrieval approach: {best_approach} with k={best_k} (score: {best_score:.3f})")
